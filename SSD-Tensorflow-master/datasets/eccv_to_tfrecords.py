...
def _process_image(directory, name):
    ...
    # filename  = '/home/z840/Desktop/ECCV-task1/VisDrone2018-DET-train/images/0000003_00231_d_0000016.jpg'

    image_data = tf.gfile.FastGFile(filename, 'rb').read()

    annoation = os.path.join(directory, DIRECTORY_ANNOTATIONS, name + '.txt')
    shape = list(Image.open(filename).size)
    bboxes = []
    labels = []
    labels_text = []
    difficult = []
    truncated = []
    for line in open(annoation):
        line = line.split(",")
        if int(line[5]) ==0 or int(line[5]) ==11 or int(line[6]) !=0 or int(line[7]) !=0 :
           line[5] =str(0)
        bboxes.append((float(line[1])/shape[1], #ymin
                        float(line[0])/shape[0],#xmin
                       (float(line[3]) + float(line[1])) / shape[1],#ymax
                       (float(line[2]) + float(line[0])) / shape[0],#xmin
                      ))
        labels.append(int(line[5]))

        labels_text.append(label_text_dic[line[5]].encode('ascii'))
        # labels_text.append(label_text_dic[label_to_4_label[line[5]]].encode('ascii'))
        difficult.append(int(line[7]))
        truncated.append(int(line[6]))
...
def _convert_to_example(image_data, labels, labels_text, bboxes, shape,
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'image/channels': int64_feature(channel),
